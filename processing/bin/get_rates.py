...
#plateFile=os.path.join(mapdir,'SO-NNR_itrf2014.txt')  # Somalian Plate Motion at sites
#plate = pd.read_csv(plateFile, delim_whitespace=True)

# load Nubian Plate Motions for ITRF14
iPlate='NUBI'
...
ITRFplate =ITRFplates[ITRFplates["plate"]==iPlate].values
iPlat,iPlon,iProt_neg = cart2sph(ITRFplate[0,1],
                             ITRFplate[0,2],
                             ITRFplate[0,3]) # cart to geog
iPlatePole=EP(iPlat,iPlon,-iProt_neg)
#except:
#    print("ITRF Euler Pole Rates not described")
...
try: 
    ...
    lPlatePole=EP(lPlat,lPlon,lProt)
except:
...
for stat in Stats:
    ...
    for comp in Comps:
        ...
        if sp == 1 :
            ...
            pAZ,pRate = VIC_ITRF.velocity(lat,lon) 
            pNvel = pRate * np.cos(np.radians(pAZ)) 
            pEvel = pRate * np.sin(np.radians(pAZ)) 
            #print("lat, lon, AZ, Rate, Nvel, Evel",stat, lat,lon,pAZ,pRate,pNvel,pEvel)
