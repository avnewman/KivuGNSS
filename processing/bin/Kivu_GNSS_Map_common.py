...
ratesFile=os.path.join(os.getcwd(),'rates','Kivu_rates_common.txt')
...
rates=pd.read_csv(ratesFile, delim_whitespace=True, comment='#')

# get the plate of record information
rfile=open(ratesFile,'r')
contents = rfile.readlines()
rfile.close()
meanListLine=contents[1]
words=meanListLine.split(' ')
meanList=words[1:-1]
#print(list(meanList))

data=rates[["Long","Lat","Evel","Nvel","Eerr","Nerr","NEcor","Uvel","STAT"]] 
# add legend velocity
vslat = float(-2.19)
vslong = float(29.38)
...
vdf[["Long","Lat","Evel","Nvel","Eerr","Nerr","NEcor","Uvel"]] = vdf[["Long","Lat","Evel","Nvel","Eerr","Nerr","NEcor","Uvel"]].apply(pd.to_numeric)
...
fig1.text(
    text=["Vertical velocity [mm/yr]", "Horizontal velocity 10 mm/yr (2 @~s@~)"],
    no_clip=True,
    x=[29.25,29.28],
    y=[-2.21,vslat]
    )
...
fig1.text(text="Observations are relative to sites:",
    x=29.20,y=-2.133, 
    no_clip=True,
    justify="LB", 
    font="9p,Helvetica,black"
    )
fig1.text(text="\("+", ".join(str(x) for x in meanList)+"\)",
    x=29.25,y=-2.145, 
    no_clip=True,
    justify="LB", 
...
#fig1.text(text="(Saria et al., 2014)", x=29.38, y=0.70, justify="LB",font="8p,Helvetica-Oblique,black" )

fig1.savefig(os.path.join(plotdir,'KIVU_GNSS_common.png'),  # types include png,jpg,pdf,bmp,tif,eps,kml
